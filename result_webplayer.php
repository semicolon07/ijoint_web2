...
$sql = "select tid, pid, side, exercise_type from task where tid = +$tid";
$query = mysqli_query($conn,$sql);
while ($row = mysqli_fetch_array($query)){
	$result_pid = $row['pid'];
	$result_tid = $row['tid'];
	$result_side = $row['side'];
	$result_exercise_type = $row['exercise_type'];
}
...
<body>
	...
	<div class="container">
		...
		<div id="body">
			...
			<!--<div><? include('include/WebPlayer.html'); ?></div>-->
